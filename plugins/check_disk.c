  ...
  for (path = path_select_list; path; path=path->name_next) {
    ...
    if (fsp.fsu_blocks && strcmp ("none", me->me_mountdir)) {
      ...
      */
      ...
      xasprintf (&output, "%sdev==%s__dused_units==%.0f__dused_pct==%.0f__",
                output,
      ...
      if (newlines) {
        if (path->dused_inodes_percent < 0) {
          ...
          xasprintf(&output, "%s inode=%.0f%%):\n", output, path->dused_inodes_percent );
        }
      } else {
        if (path->dused_inodes_percent < 0) {
          xasprintf(&output, "%sinode==;", output);
        } else {
          xasprintf(&output, "%sinode==%.0f:", output, path->dused_inodes_percent );
        }
      }
