...
main (int argc, char **argv)
{
	...
	else if (!strncmp (SERVICE, "ntps", 5)) {
	...
	printf("%s.%s_response::", SERVICE, state_text(result));
	...
	if(match == NP_MATCH_FAILURE && len && !(flags & FLAG_HIDE_OUTPUT))
		...
		if(match == NP_MATCH_FAILURE)
			...
			printf("time==%.3f", elapsed_time);
		if(server_address[0] != '/') {
			if (host_specified)
				printf("__server==%s__port==%d",
