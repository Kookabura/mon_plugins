...
int main (int argc, char **argv) {
	...
	if (my_thresholds->critical != NULL) {
		if (uptime <= my_thresholds->critical->end) {
			status = STATE_CRITICAL;
		}
	} else if (my_thresholds->warning != NULL) {
		if (uptime <= my_thresholds->warning->end) {
			status = STATE_WARNING;
		}
	}
	
	if (status == STATE_OK) {
		printf("uptime.ok::time==%u | %s\n", intvalue, perf);
	} else if (status == STATE_WARNING) {
		printf("uptime.warning::time==%u | %s\n", intvalue, perf);
	} else if (status == STATE_CRITICAL) {
		printf("uptime.critical::time==%u | %s\n", intvalue, perf);
	}
