...
* Description:
* 
* This file contains netutilss functions used in many of the plugins.
* 
* 
...
{
	if (sig == SIGALRM)
		printf (_("netutils.cri_conn_timeout::time==%d\n"),  socket_timeout);
	else
		printf (_("netutils.cri_conn_timeout::time==%d\n"), socket_timeout);
	...
	if (result == 0)
		...
		case STATE_CRITICAL: /* user did not set econn_refuse_state */
			if (is_socket)
				...
				printf("netutils.conn_failed::address==%s__port==%d\n",
...
host_or_die(const char *str)
{
	if(!str || (!is_addr(str) && !is_hostname(str)))
		usage_va(_("netutils.inv_host::host==%s"), str);
}
