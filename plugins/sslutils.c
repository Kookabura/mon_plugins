...
#  ifdef USE_OPENSSL
	...
	if (days_left > 0 && days_left <= days_till_exp_warn) {
		printf (_("http.%s_cert_valid_period::name==%s__days==%d__date==%d"), (days_left>days_till_exp_crit)?"war":"cri", cn, days_left, timegm(&stamp));
		if (days_left > days_till_exp_crit)
			return STATE_WARNING;
		else
			return STATE_CRITICAL;
	} else if (time_left < 0) {
		printf(_("http.cri_cert_exired::name==%s__date==%s"), cn, timegm(&stamp));
		status=STATE_CRITICAL;
	} else if (days_left == 0) {
		printf (_("http.%s_cert_valid_period::name==%s__days==%d__date==%d"), (days_left>days_till_exp_crit)?"war":"cri", cn, timegm(&stamp));
		if (days_left > days_till_exp_crit)
			return STATE_WARNING;
		else
			return STATE_CRITICAL;
	} else {
		printf(_("http.ok_cert_valid_period::name==%s__date==%d"), cn, timegm(&stamp));
		status=STATE_OK;
	}
