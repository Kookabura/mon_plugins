...
# endif
		...
		if (pl == 100)
			printf (_("ping.%s::warn_text==%s__pl==%d%s%s"), state_text (this_result), warn_text,
							pl,
							pl>=cpl?"__epl==1":(pl>=wpl?"__epl==1":"__epl==0"),
							rta>=crta?"__erta==1":(rta>=wrta?"__erta==1":"__erta==0"));
		else
			printf (_("ping.%s::warn_text==%s__pl==%d__rta==%2.2f%s%s"),
...
error_scan (char buf[MAX_INPUT_BUFFER], const char *addr)
{
	if (strstr (buf, "Network is unreachable") ||
		...
		die (STATE_CRITICAL, _("ping.cri_network_unreachable::host==%s"), addr);
	else if (strstr (buf, "Destination Host Unreachable"))
		die (STATE_CRITICAL, _("ping.cri_host_unreachable::host==%s"), addr);
	else if (strstr (buf, "Destination Port Unreachable"))
		die (STATE_CRITICAL, _("ping.cri_port_unreachable::host==%s"), addr);
	else if (strstr (buf, "Destination Protocol Unreachable"))
		die (STATE_CRITICAL, _("ping.cri_protocol_unreachable::host==%s"), addr);
	else if (strstr (buf, "Destination Net Prohibited"))
		die (STATE_CRITICAL, _("ping.cri_net_prohibited::host==%s"), addr);
	else if (strstr (buf, "Destination Host Prohibited"))
		die (STATE_CRITICAL, _("ping.cri_host_prohibited::host==%s"), addr);
	else if (strstr (buf, "Packet filtered"))
		die (STATE_CRITICAL, _("ping.cri_packet_filtered::host==%s"), addr);
	else if (strstr (buf, "unknown host" ))
		die (STATE_CRITICAL, _("ping.cri_not_found::host==%s"), addr);
	else if (strstr (buf, "Time to live exceeded"))
		die (STATE_CRITICAL, _("ping.cri_ttl_exceeded::host==%s"), addr);
	else if (strstr (buf, "Destination unreachable: "))
		die (STATE_CRITICAL, _("ping.cri_dest_unreachable::host==%s"), addr);
