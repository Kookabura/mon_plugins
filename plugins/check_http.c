...
main (int argc, char **argv)
{
  ...
  xasprintf (&user_agent, "User-Agent: check_http/v%s (monopus-plugins %s)",
            NP_VERSION, VERSION);
...
expected_statuscode (const char *reply, const char *statuscodes)
{
  ...
  if ((expected = strdup (statuscodes)) == NULL)
    die (STATE_UNKNOWN, _("http.unk_memory_error\n"));
...
check_document_dates (const char *headers, char **msg)
{
  ...
  if (!server_date || !*server_date) {
    xasprintf (msg, _("%shttp.unk_server_date, "), *msg);
    date_result = max_state_alt(STATE_UNKNOWN, date_result);
  } else if (!document_date || !*document_date) {
    xasprintf (msg, _("%shttp.doc_mod_date_unk, "), *msg);
    ...
    if (srv_data <= 0) {
      xasprintf (msg, _("%shttp.cri_srv_date_unparsable::date==\"%100s\", "), *msg, server_date);
      date_result = max_state_alt(STATE_CRITICAL, date_result);
    } else if (doc_data <= 0) {
      xasprintf (msg, _("%shttp.cri_doc_date_unparsable::date==\"%100s\", "), *msg, document_date);
      date_result = max_state_alt(STATE_CRITICAL, date_result);
    } else if (doc_data > srv_data + 30) {
      xasprintf (msg, _("%shttp.cri_doc_in_future::sec==%d, "), *msg, (int)doc_data - (int)srv_data);
      date_result = max_state_alt(STATE_CRITICAL, date_result);
    } else if (doc_data < srv_data - maximum_age) {
      int n = (srv_data - doc_data);
      if (n > (60 * 60 * 24 * 2)) {
        xasprintf (msg, _("%shttp.cri_last_mod_days::date==%.1f, "), *msg, ((float) n) / (60 * 60 * 24));
        date_result = max_state_alt(STATE_CRITICAL, date_result);
      } else {
        xasprintf (msg, _("%shttp.cri_last_mod::date==%d:%02d:%02d, "), *msg, n / (60 * 60), (n / 60) % 60, n % 60);
        date_result = max_state_alt(STATE_CRITICAL, date_result);
      }
    }
    free (server_date);
...
check_http (void)
{
  ...
  if (my_tcp_connect (server_address, server_port, &sd) != STATE_OK)
    die (STATE_CRITICAL, _("http.cri_err_on_conn\n"));
  ...
  */
  ...
  if (i < 0 && errno != ECONNRESET) {
    ...
    */
#endif
      die (STATE_CRITICAL, _("http.cri_err_on_recieve\n"));
#ifdef HAVE_SSL
      /* XXX
    }
    */
  ...
  if (pagesize == (size_t) 0)
    die (STATE_CRITICAL, _("http.cri_no_data\n"));
  ...
  if ( server_expect_yn  )  {
    ...
    if (strspn (status_code, "1234567890") != 3)
      die (STATE_CRITICAL, _("http.cri_inv_status_line::line==%s\n"), status_line);
    ...
    if (http_status >= 600 || http_status < 100) {
      die (STATE_CRITICAL, _("http.cri_inv_status_code::code==%s\n"), status_line);
    }
  ...
  if (strlen (regexp)) {
    ...
    if ((errcode == 0 && invert_regex == 0) || (errcode == REG_NOMATCH && invert_regex == 1)) {
      ...
      if (invert_regex == 0)
        xasprintf (&msg, _("%s__pattern==0, "), msg);
      else
        xasprintf (&msg, _("%s__pattern==1, "), msg);
      result = STATE_CRITICAL;
    }
    else {
  ...
  if (show_extended_perfdata)
    xasprintf (&msg,
           ...
           perfd_time_transfer (elapsed_time_transfer - elapsed_time_firstbyte));
  else
    xasprintf (&msg,
...
redir (char *pos, char *status_line)
{
  ...
  if (addr == NULL)
    die (STATE_UNKNOWN, _("http.unk_addr_err\n"));

  memset(addr, 0, MAX_IPV4_HOSTLENGTH);
  url = malloc (strcspn (pos, "\r\n"));
  if (url == NULL)
    die (STATE_UNKNOWN, _("http.unk_url_err\n"));
  ...
  while (pos) {
    ...
    if (i == 0) {
      ...
      if (strlen(pos) == 0)
        die (STATE_UNKNOWN,
             _("http.unk_redir_loc_not_found::redirect==%s%s\n"),
    ...
    for (; (i = strspn (pos, "\r\n")); pos += i) {
      pos += i;
      if (!(i = strspn (pos, " \t"))) {
        die (STATE_UNKNOWN, _("http.unk_redir_loc_empty::redirect==%s\n"),
             display_html ? "</A>" : "");
      }
    }
    ...
    /* URI_PATH */
      ...
      die (STATE_UNKNOWN,
           _("http.unk_refir_parsing_err::redirect%s%s\n"),
           pos, (display_html ? "</A>" : ""));
    }
  ...
  if (++redir_depth > max_depth)
    die (STATE_WARNING,
         _("http.war_max_redir_depth::depth==%d__url==%s://%s:%d%s%s\n"),
  ...
  if (server_port==i &&
    ...
    die (STATE_WARNING,
         _("http.war_redir_loop::url==%s://%s:%d%s%s\n"),
  ...
  if (server_port > MAX_PORT)
    die (STATE_UNKNOWN,
         _("http.unk_redir_to_port::port==%d__url==%s://%s:%d%s%s\n"),
